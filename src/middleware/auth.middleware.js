...
function authUserToken(req, res, next) {
  ...
  if (decode == null) {
    throw new UnauthorizedError('Access Denied, no auth token');
  }
  logger.debug(
    `Extracted Auth Token, ID: ${decode.payload.id} name: ${decode.payload.username} email:${decode.payload.email}`
  );
